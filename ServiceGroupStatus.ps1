...
function sgstatus {
    ...
    IF ($connected) {
        $servicewild = $service + "*"
        TRY {
            $sgbind = Invoke-Nitro -Method GET -Type servicegroup -Confirm -Force -ErrorAction Continue | select servicegroup -ExpandProperty servicegroup | select servicegroupname, servicetype, state, servicegroupeffectivestate | Where-Object {($_.servicegroupname -like $servicewild)}
            IF ($sgbind -isnot [System.Array]) {
                $sgbind = @($sgbind)
                }#IF sgbind array
            ForEach ($sgbndarray in $sgbind) {
                    $sgmembers = Invoke-Nitro -Method GET -Type servicegroup_servicegroupmember_binding -resource $sgbndarray.servicegroupname -Confirm -Force -ErrorAction Continue -OnErrorAction CONTINUE
                    ForEach ($sgbound in $sgmembers.servicegroup_servicegroupmember_binding) {
                            $sgobj = New-Object -TypeName PSObject
                            Add-Member -InputObject $sgobj -MemberType NoteProperty -Name VPX -Value $nsipaddr
                            Add-Member -InputObject $sgobj -MemberType NoteProperty -Name SGName -Value $sgbound.servicegroupname
                            Add-Member -InputObject $sgobj -MemberType NoteProperty -Name ServerName -Value $sgbound.servername
                            Add-Member -InputObject $sgobj -MemberType NoteProperty -Name SrvIP -Value $sgbound.ip
                            Add-Member -InputObject $sgobj -MemberType NoteProperty -Name Port -Value $sgbound.port
                            Add-Member -InputObject $sgobj -MemberType NoteProperty -Name State -Value $sgbound.svrstate
                            Add-Member -InputObject $sgobj -MemberType NoteProperty -Name Enabled -Value $sgbound.state
                            $output +=  $sgobj
                    }#For each servicegroup
            }#ForEach $sgbndarray
        }#TRY Get ServiceGroups
...
$serviceinput = Read-Host -Prompt "ServiceGroup?"
...
ForEach ($nsip in $nmasdevices) {
    TRY {
        ...
        $report += sgstatus $fqdn.namehost $serviceinput
    }#TRY DNS Resolve
    CATCH {
          $report += sgstatus $nsip.ip_address $serviceinput
          #sgstatus $nsip
    }#CATCH DNS Resolve
} #ForEach nsip
...
"@
$rpttitle = "Service Group Status"
$rptpre = "<H1>Service Group Status</H1>"
$rptpst = "*** End of Report ***"

$report | Sort-Object -Property nsip | ConvertTo-Html -Title $rpttitle -Head $rpthdr -PreContent $rptpre -PostContent $rptpst | Set-CellColor -Property Enabled -Row -Color red -Filter "Enabled -eq 'DISABLED'" | Set-CellColor -Property State -Color yellow -Filter "State -ne 'UP'" | Out-File H:\status.html
