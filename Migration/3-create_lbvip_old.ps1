...
$logfile = "H:\NS-Migration\" + $env + "-VIPoutput.txt"
...
#ServiceGroup Members
            ...
            CATCH {
                  "$(Get-TimeStamp) FAILED: Get ServiceGroup Member for " + $oldvsbindings.lbvserver_binding.lbvserver_servicegroup_binding.servicegroupname + " " + $_.Exception.Message | Out-File -filepath $logfile -Append -Encoding ascii
            }#CATCH Get ServiceGroup Members
            $oldsrvmem = @()
            ForEach ($oldsrvname in $oldsvgbnd) {
                TRY {
                    $oldsrvmem += Get-NSLBServer -Name $oldsrvname.servername
                    }#TRY Get Server
                CATCH {
                      "$(Get-TimeStamp) FAILED: Get Server for " + $oldsvgbnd.servername + " " + $_.Exception.Message | Out-File -filepath $logfile -Append -Encoding ascii
                      }#CATCH Get Server
            }#ForEach $oldsrvname
...
#Responder Policy
        IF ($oldvsbindings.lbvserver_binding.lbvserver_responderpolicy_binding -ne $null) {
            ...
            Catch {
                  "$(Get-TimeStamp) FAILED: Get Responder binding for " + $oldvipname + " " + $_.Exception.Message | Out-File -filepath $logfile -Append -Encoding ascii
            ...
            Catch {
                  "$(Get-TimeStamp) FAILED: Get Responder Policy for " + $oldvsbindings.lbvserver_binding.lbvserver_responderpolicy_binding.policyname + " " + $_.Exception.Message | Out-File -filepath $logfile -Append -Encoding ascii
            }#CATCH Get Responder Policy
            Try {
                $oldrspact = Get-NSResponderAction -Name $oldrsppol.action
            }#TRY Get Responder Policy
            Catch {
                  "$(Get-TimeStamp) FAILED: Get Responder Action for " + $oldrsppol.action + " " + $_.Exception.Message | Out-File -filepath $logfile -Append -Encoding ascii
            }#CATCH Get Responder Policy
        }#If Responder Policy
...
#Rewrite Policy
        IF ($oldvsbindings.lbvserver_binding.lbvserver_rewritepolicy_binding -ne $null) {
            ...
            Catch {
                  "$(Get-TimeStamp) FAILED: Get Rewrite binding for " + $oldvipname + " " + $_.Exception.Message | Out-File -filepath $logfile -Append -Encoding ascii
            ...
            Catch {
                  "$(Get-TimeStamp) FAILED: Get Rewrite Policy for " + $oldvsbindings.lbvserver_binding.lbvserver_rewritepolicy_binding.policyname + " " + $_.Exception.Message | Out-File -filepath $logfile -Append -Encoding ascii
            }#CATCH Get Rewrite Policy
            Try {
                $oldrewact = Get-NSRewriteAction -Name $oldrewpol.action
            }#TRY Get Rewrite Policy
            Catch {
                  "$(Get-TimeStamp) FAILED: Get Rewrite Action for " + $oldrewpol.action + " " + $_.Exception.Message | Out-File -filepath $logfile -Append -Encoding ascii
            }#CATCH Get Rewrite Policy
        }#If Rewrite Policy
...
#Connect New NetScaler
        ...
        IF ($oldvserver -ne $null) {
#Create New VIP
            "$(Get-TimeStamp) CREATE: New VIP " + $oldvserver.lbvserver.name | Out-File -filepath $logfile -Append -Encoding ascii
            TRY {
                Invoke-Nitro -Method POST -Type lbvserver -OnErrorAction CONTINUE -Confirm -Force -Payload $newvippayld
            }#TRY Create VIP
            CATCH {
                  "$(Get-TimeStamp) FAILED: Create LB VIP for " + $oldvserver.lbvserver.name + " " + $_.Exception.Message | Out-File -filepath $logfile -Append -Encoding ascii
            }#CATCH Create VIP
#
...
#Set SSL Profile/Cert to VIP
		    IF ($oldvserver.lbvserver.servicetype -eq "SSL") {
                "$(Get-TimeStamp) BIND: SSL Record " + $oldvserver.lbvserver.name + " " + $oldsslbind.sslvserver_sslcertkey_binding.certkeyname | Out-File -filepath $logfile -Append -Encoding ascii
                $newsslprfpayld = @{ }
                $newsslprfpayld = @{
...
#				    Add-NSLBSSLVirtualServerCertificateBinding -Certificate $oldsslbind.sslvserver_sslcertkey_binding.certkeyname -VirtualServerName $oldvserver.lbvserver.name -ErrorAction CONTINUE -Confirm
	    		...
	    		CATCH {
                  "$(Get-TimeStamp) FAILED: Set SSL Cert Binding for " + $oldsslbind.vservername + " " + $oldsslbind.sslvserver_sslcertkey_binding.certkeyname + " " + $_.Exception.Message | Out-File -filepath $logfile -Append -Encoding ascii
			    }#CATCH SSL Set Profile
		    }#IF VIP is SSL
#Bind Service to VIP
            IF ($oldvsbindings.lbvserver_binding.lbvserver_service_binding -ne $null) {
                "$(Get-TimeStamp) BIND: Service Record " + $oldvserver.lbvserver.name + " " + $newsvcname | Out-File -filepath $logfile -Append -Encoding ascii
                $newsvcvippayld = @{ }
                $newsvcvippayld = @{
                                   name = $oldvserver.lbvserver.name
...
#		    		Add-NSLBVirtualServerBinding -VirtualServerName $oldvsbindings.vservername -ServiceName $newsvcname -ErrorAction CONTINUE -Confirm
    			...
    			CATCH {
                  "$(Get-TimeStamp) FAILED: Service Binding for " + $oldvserver.lbvserver.name + " " + $newsvcname + " " + $_.Exception.Message | Out-File -filepath $logfile -Append -Encoding ascii
		    	}#CATCH Service Binding
            }#If Bind Service

#Create New ServiceGroup
            IF ($oldvsbindings.lbvserver_binding.lbvserver_servicegroup_binding -ne $null) {
                "$(Get-TimeStamp) BIND: ServiceGroup Record " + $oldvsbindings.vservername + " " + $newsvgname | Out-File -filepath $logfile -Append -Encoding ascii
                $newsvgpayld = @{ }
                $newsvgpayld = @{
...
#		    		New-NSLBServiceGroup -name $newsvgname -ServiceType $newsvgtype -comment $newsvgcomment -ClientIP $newsvgcip -ClientIPHeader $newsvgciphead -Confirm -ErrorAction Continue
                ...
                CATCH {
                  "$(Get-TimeStamp) FAILED: ServiceGroup Create for " + $newsvgname + " " + $_.Exception.Message | Out-File -filepath $logfile -Append -Encoding ascii
                }#CATCH New Service Group
                    
#Create New Servers
                ForEach ($svgmemcnt in $oldsrvmem) {
                    #Create new LB Server
                    "$(Get-TimeStamp) CREATE: New SERVER Record " + $svgmemcnt.name | Out-File -filepath $logfile -Append -Encoding ascii
                    $newsrvpayld = @{ }
                    $newsrvpayld = @{
                                        name = $svgmemcnt.name
                                        domain = $svgmemcnt.domain
                                        } #newsrvpayld
				    TRY {
...
#					    New-NSLBServer -confirm -name $svgmemcnt.servername -IPAddress $svgmemcnt.ip -ErrorAction Continue
	    			...
	    			CATCH {
                          "$(Get-TimeStamp) FAILED: Server Create for " + $svgmemcnt.name + " " + $_.Exception.Message | Out-File -filepath $logfile -Append -Encoding ascii
			    	}#CATCH New Server
#Bind ServiceGroup Member
                    "$(Get-TimeStamp) BIND: New SERVER Record " + $svgmemcnt.name + " to " + $oldsvgbnd.servicegroupname | Out-File -filepath $logfile -Append -Encoding ascii
                    $newsvgmempayld = @{ }
                    $newsvgmempayld = @{
                                        servicegroupname = $oldsvgbnd.servicegroupname
                                        servername = $svgmemcnt.name
                                        port = $oldsvgbnd.port
                                        weight = $oldsvgbnd.weight
                    }#newsvgmempayld
    				TRY {
...
#	    				New-NSLBServiceGroupMember -name $svgmemcnt.servicegroupname -ServerName $svgmemcnt.servername -Port $svgmemcnt.port -Weight $svgmemcnt.weight -Confirm -ErrorAction Continue
			    	...
			    	CATCH {
                      "$(Get-TimeStamp) FAILED: Service Group Member Binding for " + $svgmemcnt.name + " to " + $oldsvgbnd.servicegroupname + $_.Exception.Message | Out-File -filepath $logfile -Append -Encoding ascii
    				}#CATCH New Server
...
#Bind ServiceGroup SSLProfileBackend
                IF ($newsvgtype -eq "SSL") {
			        ...
			        CATCH {
                          "$(Get-TimeStamp) FAILED: ServiceGroup SSL Profile Binding for " + $newsvgname + " " + $_.Exception.Message | Out-File -filepath $logfile -Append -Encoding ascii
           		    }#CATCH SSL Profile Backend
                }#IF ServiceGroup SSL
#Bind ServiceGroup to VIP
                "$(Get-TimeStamp) BIND: ServiceGroup to VIP " + $newsvgname + " " + $oldvserver.lbvserver.name | Out-File -filepath $logfile -Append -Encoding ascii
                $newsvgvippayld = @{ }
                $newsvgvippayld = @{
                                   name = $oldvserver.lbvserver.name
                                   servicegroupname = $newsvgname
...
#		    		Add-NSLBVirtualServerBinding -VirtualServerName $oldvserver.lbvserver.name -ServiceGroupName $newsvgname -ErrorAction CONTINUE -Confirm
	    		...
	    		CATCH {
                      "$(Get-TimeStamp) FAILED: ServiceGroup VIP Binding for " + $newsvgname + " " + $oldvserver.lbvserver.name + " " + $_.Exception.Message | Out-File -filepath $logfile -Append -Encoding ascii
...
#Create Responder Action
            IF ($oldvsbindings.lbvserver_binding.lbvserver_responderpolicy_binding -ne $null) {
                "$(Get-TimeStamp) CREATE: Responder Action " + $oldrspact.name | Out-File -filepath $logfile -Append -Encoding ascii
...
#			    	    New-NSResponderAction -Name $oldrspact.name -Type $oldrspact.type -Target $oldrspact.target -ResponseStatusCode $oldrspact.responsestatuscode -Confirm -ErrorAction Continue
    			    ...
    			    CATCH {
                          "$(Get-TimeStamp) FAILED: New Responder Action for " + $oldrspact.name + " " + $_.Exception.Message | Out-File -filepath $logfile -Append -Encoding ascii
    	    		}#CATCH Responder Action

#Create Responder Policy
                   "$(Get-TimeStamp) CREATE: Responder Policy " + $oldrsppol.name | Out-File -filepath $logfile -Append -Encoding ascii
                    $newrsppolpayld = @{ }
                    $newrsppolpayld = @{
                                    name = $oldrsppol.name
                                    rule = $oldrsppol.rule
...
#    		    		New-NSResponderPolicy -Name $oldrsppol.name -Rule $oldrsppol.rule -Action $oldrsppol.action -Confirm -ErrorAction Continue
		    	    ...
		    	    CATCH {
                          "$(Get-TimeStamp) FAILED: New Responder Policy for " + $oldrsppol.name + " " + $_.Exception.Message | Out-File -filepath $logfile -Append -Encoding ascii
...
#Bind Responder Policy
                ...
                "$(Get-TimeStamp) BIND: Responder Policy " + $oldrsppol.name + " " + $oldvserver.lbvserver.name | Out-File -filepath $logfile -Append -Encoding ascii
    			TRY {
			    ...
			    CATCH {
                      "$(Get-TimeStamp) FAILED: Responder Binding for " + $oldrsppol.name + " " + $oldvserver.lbvserver.name + " " + $_.Exception.Message | Out-File -filepath $logfile -Append -Encoding ascii
...
#Create Rewrite Action
            IF ($oldvsbindings.lbvserver_binding.lbvserver_rewritepolicy_binding -ne $null) {
                ...
                "$(Get-TimeStamp) CREATE: Rewrite Action " + $oldrewact.name | Out-File -filepath $logfile -Append -Encoding ascii
	    		TRY {
...
#>
			    ...
			    CATCH {
                      "$(Get-TimeStamp) FAILED: New Rewrite Action for " + $oldrewact.name + " " + $_.Exception.Message | Out-File -filepath $logfile -Append -Encoding ascii
	    		}#CATCH Rewrite Action
#Create Rewrite Policy
                "$(Get-TimeStamp) CREATE: Rewrite Policy " + $oldrewpol.name | Out-File -filepath $logfile -Append -Encoding ascii
                    $newrewpolpayld = @{ }
                    $newrewpolpayld = @{
                                    name = $oldrewpol.name
                                    rule = $oldrewpol.rule
...
#>
    			...
    			CATCH {
                      "$(Get-TimeStamp) FAILED: New Rewrite Policy for " + $oldrewpol.name + " " + $_.Exception.Message | Out-File -filepath $logfile -Append -Encoding ascii
	    		}#CATCH Rewrite Policy

#Bind Rewrite Policy
                "$(Get-TimeStamp) BIND: Rewrite Policy " + $oldrewpol.name + " " + $oldvserver.lbvserver.name | Out-File -filepath $logfile -Append -Encoding ascii
    			TRY {
			    ...
			    CATCH {
                      "$(Get-TimeStamp) FAILED: Rewrite Binding for " + $oldrewpol.name + " " + $oldvserver.lbvserver.name + " " + $_.Exception.Message | Out-File -filepath $logfile -Append -Encoding ascii
	    		}#CATCH Rewrite Bind
            }#IF Rewrite Policy
        }# IF VIP not Null
        ELSE {
